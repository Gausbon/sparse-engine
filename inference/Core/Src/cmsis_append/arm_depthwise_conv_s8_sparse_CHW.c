...
# define DW_BLOCK 3
void arm_nn_dw_convolve_s8_single_sparse_CHW ( const cmsis_nn_dw_conv_params *dw_conv_params,
                                    ...
                                    q31_t *buffer)
{
    ...
    int32_t k_y, k_x;
    const q7_t *input_ptr;

    input_ptr = input_data + in_ch * input_x * input_y;
        ...
        if (k_y >= 0 && k_y < input_y) {
            for (int32_t i_out_x = 0; i_out_x < output_x; i_out_x++) {
                if (k_x >= 0 && k_x < input_x) {
                    // not padding
                    (*buffer) += ((input_ptr[k_x] + in_offset) * ker_val);
                }
                buffer++;
                k_x += stride_x;
            }
...
void arm_nn_dw_convolve_s8_double_sparse_CHW ( const cmsis_nn_dw_conv_params *dw_conv_params,
                                    const q7_t *input_data,
                                    const int32_t input_x,
                                    const int32_t input_y,
                                    const int32_t output_x,
                                    ...
                                    q31_t *buffer)
{
    ...
    input_ptr_0 = input_data + in_ch_0 * input_x * input_y;
    input_ptr_1 = input_data + in_ch_1 * input_x * input_y;
    ...
    for (int32_t i_out_y = 0; i_out_y < output_y; i_out_y++) {
        ...
        for (int32_t i_out_x = 0; i_out_x < output_x; i_out_x++) {
            if (k_y_0 >= 0 && k_y_0 < input_y && k_x_0 >= 0 && k_x_0 < input_x) {
                input_0 = input_ptr_0[k_x_0];
            } else {
                input_0 = -in_offset;
            }

            if (k_y_1 >= 0 && k_y_1 < input_y && k_x_1 >= 0 && k_x_1 < input_x) {
                input_1 = input_ptr_1[k_x_1];
            } else {
                input_1 = -in_offset;
            }
        ...
        while (cnt) {
            ...
            if (mat_flag) {   
                ...
                if (double_flag == 1 && last_val != 0) {
                    ...
                    arm_nn_dw_convolve_s8_single_sparse_CHW(dw_conv_params,
                        in_ptr,
                        input_x, input_y,
                        output_x, output_y,
                        output_ch,
            ...
            if (double_flag && !(last_val == 0 && cur_val == 0)) {
                ...
                // in depthwise, out_ch replace in_ch
                arm_nn_dw_convolve_s8_double_sparse_CHW(dw_conv_params,
                        in_ptr,
                        input_x, input_y,
                        output_x, output_y,
                        output_ch,

                            ...
                            const int32_t input_count)
{
    ...
    while (filter_ptr < end_ptr) {
        ...
        if (mat_flag) {
            ...
            if (two_count == 1 && last_val != 0) {
                ...
                in_ptr_0 = input_data + last_in_ch * batch;
                for (int32_t i_batch = 0; i_batch < batch; ++i_batch) {
                    *buffer += ((*in_ptr_0++ + in_offset) * last_val);
                    buffer++;
                }
            }
            two_count = 0;
            ...
            out_ptr = output_data + last_out_ch * batch;
        ...
        if (two_count && !(last_val == 0 && cur_val == 0)) {
            ...
            in_ptr_0 = input_data + last_in_ch * batch;
            in_ptr_1 = input_data + cur_in_ch * batch;
            for (int32_t i_batch = 0; i_batch < batch; ++i_batch) {
    ...
    if (two_count == 1 && last_val != 0) {
        // remain one val to conv (last_val)
        buffer = (int32_t*)ctx->buf;
        in_ptr_0 = input_data + cur_in_ch * batch;
        for (int i_batch = 0; i_batch < batch; ++i_batch) {
            *buffer += ((*in_ptr_0++ + in_offset) * cur_val);
            buffer++;
        }
    }
    ...
    out_ptr = output_data + cur_out_ch * batch;
